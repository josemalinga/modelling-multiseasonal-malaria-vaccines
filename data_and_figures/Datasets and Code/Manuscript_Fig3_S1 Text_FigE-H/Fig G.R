...
# Working directory
setwd(paste0("/scicore/home/penny/",user,"/M3TPP/Experiments/round1_plots/Manuscript_Fig3"))

##########################################################################################
### Visualizes tradeoffs and reduction for hybrid vaccination and mass vaccination
##########################################################################################

data_set <- readRDS(paste0(getwd(),"/data_FigGa.rds"))
...
data_set$access = recode_factor(data_set$access,"0.1227" ="Moderate access to care",
                              "0.4165" ="High access to care")

data_set$expr = recode_factor(data_set$exp,"Moderate" ="Moderate access to care",
                              "High" ="High access to care")
...
##################
data_set <- readRDS(paste0(getwd(),"/data_FigGb.rds"))

data_set$Seasons = recode_factor(data_set$Seasons,"Seasonal transmission" ="Short season",
                                 "LS transmission" ="Long season", "Perennial transmission"="Constant transmission")
...
ggsave(filename = paste0(getwd(),"/","Fig G.pdf"),
       plot = ab,
       width = 18,
       height = 10,
       dpi = 300)
