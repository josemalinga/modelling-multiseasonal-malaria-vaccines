...
# Working directory
setwd(paste0("/scicore/home/penny/",user,"/M3TPP/Experiments/round1_plots/Manuscript_Fig4"))

##########################################################################################
### Visualizes tradeoffs and reduction for hybrid vaccination and mass vaccination
##########################################################################################

data_set <- readRDS(paste0(getwd(),"/data_FigKa.rds"))

data_set$expr = recode_factor(data_set$exp,"Moderate" ="Hybrid vaccination",
                              "ModTimed" ="Mass vaccination")
...
data_set$access = recode_factor(data_set$access_cat,"0.1227" ="Moderate access to care",
                                "0.4165" ="High access to care")
...
a<-ggplot(data = data_set, aes(x = prev_cat, y = important, fill = Factors)) +
  ...
  facet_nested(outcome~access, scales = "free_y")+
...
##################
data_set <- readRDS(paste0(getwd(),"/data_FigKb.rds"))

data_set$expr = recode_factor(data_set$exp,"Moderate" ="Hybrid vaccination",
                              "ModTimed" ="Mass vaccination")

data_set$Seasons = data_set$season

data_set$access = recode_factor(data_set$access_cat,"0.1227" ="Moderate access to care",
                                "0.4165" ="High access to care")

data_set0 <- data_set[data_set$Seasons=="Short season" &
                        data_set$out!="24m5" &
...
ggsave(filename = paste0(getwd(),"/","Fig K.pdf"),
       plot = ab,
       width = 18,
       height = 10,
       dpi = 300)
